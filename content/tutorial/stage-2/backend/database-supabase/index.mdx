...
![](/tutorial/images/stage-2/backend/database-supabase/images/image1.png)
...
![](/tutorial/images/stage-2/backend/database-supabase/images/image6.png)![](/tutorial/images/stage-2/backend/database-supabase/images/image7.png)
...
![](/tutorial/images/stage-2/backend/database-supabase/images/image8.png)
...
![](/tutorial/images/stage-2/backend/database-supabase/images/image9.png)

如果你想要手动新建 table，可以在 tables 中直接新建表格，我们会在之后的教程中详细讲解。

![](/tutorial/images/stage-2/backend/database-supabase/images/image10.png)
...
![](/tutorial/images/stage-2/backend/database-supabase/images/image11.png)

你可以在 Provider 选项中找到不同 supabase 支持的用户信息登录入口，默认使用 Email；如果你想使用 Github 或者 Google 账户进行登录，还需要更多属性配置，我们会在下面的课程中进行详细讲解。

![](/tutorial/images/stage-2/backend/database-supabase/images/image12.png)

在 Sign In / Providers 里还包含了对注册邮箱行为的控制，如果你不想每次邮箱注册都必须让用户接受邀请后才能成为用户，你可以取消 Confirm email 的强制要求。

![](/tutorial/images/stage-2/backend/database-supabase/images/image13.png)

如果你想切换非 Supabase 的其他 auth 系统服务商，你可以点击 Third Party Auth，比如此处就使用 Clerk 作为第三方的系统服务商。

![](/tutorial/images/stage-2/backend/database-supabase/images/image14.png)

如果你担心注册用户在短期内访问量过大，你可以在 Rate Limits 中启用对应的流量限制策略：

![](/tutorial/images/stage-2/backend/database-supabase/images/image15.png)
...
![](/tutorial/images/stage-2/backend/database-supabase/images/image16.png)

我们将会在本节课的进阶 project 中讲解 storage 的具体用法。

![](/tutorial/images/stage-2/backend/database-supabase/images/image17.png)

如果你想使用 S3 的相关协议进行操作，可以直接使用对应的配置：

![](/tutorial/images/stage-2/backend/database-supabase/images/image18.png)
...
![](/tutorial/images/stage-2/backend/database-supabase/images/image19.png)
...
![](/tutorial/images/stage-2/backend/database-supabase/images/image20.png)

Edge Functions 的函数使用 secrets 中暴露的密钥作为环境变量，通过 `Deno.env.get` 加载，从而实现第三方服务的调用。这样一来，敏感密钥就永远不会暴露在客户端（你的浏览器），彻底杜绝了被盗用的风险。

![](/tutorial/images/stage-2/backend/database-supabase/images/image21.png)
...
![](/tutorial/images/stage-2/backend/database-supabase/images/image22.png)
...
![](/tutorial/images/stage-2/backend/database-supabase/images/image23.png)
...
![](/tutorial/images/stage-2/backend/database-supabase/images/image24.png)
...
![](/tutorial/images/stage-2/backend/database-supabase/images/image25.png)

值得注意的是，对应数据表的名称以及存储的数据类型可在下方的 Columns 中指定。

![](/tutorial/images/stage-2/backend/database-supabase/images/image26.png)

对于关系数据库，其中很重要的特点是表与表之间的关联，你可以在下方找到 `Foreign keys` ，点击创建相应的关联关系：

![](/tutorial/images/stage-2/backend/database-supabase/images/image27.png)
...
![](/tutorial/images/stage-2/backend/database-supabase/images/image28.png)
...
![](/tutorial/images/stage-2/backend/database-supabase/images/image29.png)
...
![](/tutorial/images/stage-2/backend/database-supabase/images/image30.png)
...
![](/tutorial/images/stage-2/backend/database-supabase/images/image31.png)
...
![](/tutorial/images/stage-2/backend/database-supabase/images/image32.png)

![](/tutorial/images/stage-2/backend/database-supabase/images/image33.png)

![](/tutorial/images/stage-2/backend/database-supabase/images/image34.png)

主动配置难免显得麻烦，通常，我们在数据表语句创建、初始化的时候就会自动考虑植入对应的 RLS 策略。我们只需在 SQL Editor 中执行类似如下语句，即可自动开启对应数据表的行级安全策略。

![](/tutorial/images/stage-2/backend/database-supabase/images/image35.png)
...
![](/tutorial/images/stage-2/backend/database-supabase/images/image36.png)
...
![](/tutorial/images/stage-2/backend/database-supabase/images/image37.png)

![](/tutorial/images/stage-2/backend/database-supabase/images/image38.png)
...
![](/tutorial/images/stage-2/backend/database-supabase/images/image39.png)
...
![](/tutorial/images/stage-2/backend/database-supabase/images/image40.png)

但此时点击 admin，你并不能看到如下界面，你需要尝试在数据表中找到控制用户权限的部分，将权限修改为 `admin`，从而能够在 Admin 界面正常看到如下内容：

![](/tutorial/images/stage-2/backend/database-supabase/images/image41.png)
...
![](/tutorial/images/stage-2/backend/database-supabase/images/image42.png)
...
![](/tutorial/images/stage-2/backend/database-supabase/images/image43.png)
...
![](/tutorial/images/stage-2/backend/database-supabase/images/image44.png)
...
![](/tutorial/images/stage-2/backend/database-supabase/images/image45.png)
...
![](/tutorial/images/stage-2/backend/database-supabase/images/image46.png)

![](/tutorial/images/stage-2/backend/database-supabase/images/image47.png)
...
![](/tutorial/images/stage-2/backend/database-supabase/images/image48.png)
...
![](/tutorial/images/stage-2/backend/database-supabase/images/image49.png)
...
15. 在 “Authorized redirect URIs” 部分，点击 “ADD URI”，并填入你的 Supabase 项目的回调 URL。你可以在 Supabase Dashboard 的 “Authentication” -> “Providers” -> “Google” 中找到这个 URL，它的格式通常是 `https://&lt;你的项目ID>.supabase.co/auth/v1/callback`。
    ![](/tutorial/images/stage-2/backend/database-supabase/images/image50.png)
16. 点击 “CREATE”。
...
9. 注册成功后，页面会显示你的 **Client ID** 。
   ![](/tutorial/images/stage-2/backend/database-supabase/images/image51.png)
...
![](/tutorial/images/stage-2/backend/database-supabase/images/image52.png)
...
![](/tutorial/images/stage-2/backend/database-supabase/images/image53.png)
...
![](/tutorial/images/stage-2/backend/database-supabase/images/image54.png)
...
![](/tutorial/images/stage-2/backend/database-supabase/images/image55.png)

![](/tutorial/images/stage-2/backend/database-supabase/images/image56.png)
...
![](/tutorial/images/stage-2/backend/database-supabase/images/image57.png)
...
5. 点击 “Create a new function” 按钮。
   ![](/tutorial/images/stage-2/backend/database-supabase/images/image58.png)
6. 为函数命名，例如 `llm-chat`。
7. **粘贴代码** :
   ![](/tutorial/images/stage-2/backend/database-supabase/images/image59.png)
8. 在弹出的在线编辑器中， **删除所有默认的占位代码** 。
9. 打开你本地的 `llm-chat.ts` 文件， **复制其全部内容** 。
10. 将复制的代码**粘贴**到 Supabase 的在线编辑器中。
11. **配置\*\***环境变量\*\* ** (Secrets)** :
    1. 在侧边栏找到 Secrets。
       ![](/tutorial/images/stage-2/backend/database-supabase/images/image60.png)
...
![](/tutorial/images/stage-2/backend/database-supabase/images/image61.png)
...
1. 注册与创建:
   ...
   2. 点击 "Create application" 。
      ![](/tutorial/images/stage-2/backend/database-supabase/images/image62.png)
...
2. 获取 API Keys:
   1. 创建成功后，你会被引导至 API Keys 页面。
      ![](/tutorial/images/stage-2/backend/database-supabase/images/image63.png)
   2. 找到 Publishable key (以 `pk_` 开头) 和 Secret key (以 `sk_` 开头)。
      ![](/tutorial/images/stage-2/backend/database-supabase/images/image64.png)
...
![](/tutorial/images/stage-2/backend/database-supabase/images/image65.png)
...
- 2.2 Google 平台配置：
  ...
  - 关键注意事项：
    ...
    4. 支持 Google One Tap：配置完成后，可集成 Clerk `<div class="vue-demo-placeholder" data-demo="GoogleOneTap">
<p>📦 Interactive Demo: <strong>GoogleOneTap</strong></p>
<p>This interactive visualization is not available in the static version.</p>
<a href="https://datawhalechina.github.io/easy-vibe/zh-cn/stage-2/backend/database-supabase/" target="_blank" rel="noopener noreferrer" class="demo-original-link">🔗 View original interactive demo →</a>
</div>` 组件实现“一键登录”，参考 [Clerk 组件文档](https://clerk.com/docs/components/social-connections/google-one-tap)。
