...
2. 双击安装程序并按照安装向导说明进行操作：
   ![](/tutorial/images/stage-2/backend/git-workflow/images/image5.webp)
   1. 建议保持默认选项。如果你需要自定义，请注意以下几点：（在大多数情况下，你可以一直点击"Next"）
      - 选择 Git 使用的默认编辑器：选择你喜欢的编辑器（如 VS Code）。你可以默认选择第一个选项，即 Vim（一个文本编辑器），或选择"Visual Studio Code as Git's default editor"选项（需要预先安装 VS Code）。你可以保持默认选择并点击"Next"继续。
        ![](/tutorial/images/stage-2/backend/git-workflow/images/image6.webp)
      - 选择如何使用 Git：这三个选项控制 Git 在系统中的可访问性。建议选择选项 2（"from command line and 3rd-party software"）——它将基本的 Git 工具添加到 PATH 中，让你可以在 Git Bash、命令提示符、PowerShell 和 IDE 中使用 Git，而不会使系统混乱。
        ![](/tutorial/images/stage-2/backend/git-workflow/images/image7.webp)

3. 安装后，在桌面上右键单击。如果在菜单中看到"Git Bash Here"，则安装成功。

![](/tutorial/images/stage-2/backend/git-workflow/images/image8.webp)
...
![](/tutorial/images/stage-2/backend/git-workflow/images/image9.webp)

许多大公司在 GitHub 上开源他们的程序或教程，以获得行业竞争优势——这也可以看作是一种广告形式。在 GitHub 社区中，项目获得的"星标 (stars)"数量是衡量其价值的主要指标；项目或组织拥有的星标越多，其可信度和影响力就越大。

![](/tutorial/images/stage-2/backend/git-workflow/images/image10.webp)
...
1. 访问 [GitHub 官网](https://github.com/) 并点击右上角的"Sign up"。
   ![](/tutorial/images/stage-2/backend/git-workflow/images/image11.webp)
...
![](/tutorial/images/stage-2/backend/git-workflow/images/image12.webp)![](/tutorial/images/stage-2/backend/git-workflow/images/image13.webp)

![](/tutorial/images/stage-2/backend/git-workflow/images/image14.webp)
...
![](/tutorial/images/stage-2/backend/git-workflow/images/image15.webp)

之后，你将拥有一个没有任何额外文件的干净仓库。接下来你可以开始上传文件了。

![](/tutorial/images/stage-2/backend/git-workflow/images/image16.webp)

获取仓库的命令是 `git clone`，但它需要仓库地址。你可以通过点击绿色的"Code"按钮找到仓库地址，你会看到 HTTPS 和 SSH 选项。通常，你可以使用这两种方法中的任何一种将仓库下载到本地机器（只有这样你才能修改和上传文件）。

![](/tutorial/images/stage-2/backend/git-workflow/images/image17.webp)
...
1. 本地生成 SSH 密钥对
   ...
   ![](/tutorial/images/stage-2/backend/git-workflow/images/image18.webp)

   输入提示词后，你还需要在左侧终端按 Enter 键，否则命令会一直等待而不执行。由于 Trae 无法帮你执行任何条件判断，我们只需要一直按 Enter 即可。

   最后，你会看到右侧的 Trae 返回了它读取的公钥。你只需复制它并准备在下一步中粘贴。

   ![](/tutorial/images/stage-2/backend/git-workflow/images/image19.webp) 2. 手动获取公钥
...
2. 登录 GitHub 并进入"SSH Key Management"页面：
   1. 点击右上角头像 → Settings → 左侧菜单 SSH and GPG keys → 点击 New SSH key。
      ![](/tutorial/images/stage-2/backend/git-workflow/images/image20.webp)![](/tutorial/images/stage-2/backend/git-workflow/images/image21.webp)
   2. 输入任何标题（例如，your local computer's SSH），然后将你刚刚获取的 SSH 公钥粘贴到这里。

![](/tutorial/images/stage-2/backend/git-workflow/images/image22.webp)

![](/tutorial/images/stage-2/backend/git-workflow/images/image23.webp)
...
![](/tutorial/images/stage-2/backend/git-workflow/images/image24.webp)
...
![](/tutorial/images/stage-2/backend/git-workflow/images/image25.webp)
