...
![](/tutorial/images/stage-2/backend/zeabur-deployment/images/image1.webp)
...
1. **服务器准备**：你需要先购买云服务器（比如阿里云、腾讯云、或 AWS EC2），选择服务器所在地区（如上海、新加坡）、配置（CPU、内存、磁盘大小等），还要学会如何远程连接服务器（例如通过 SSH 工具登录）。
   ![](/tutorial/images/stage-2/backend/zeabur-deployment/images/image2.webp)
...
![](/tutorial/images/stage-2/backend/zeabur-deployment/images/image3.webp)
...
![](/tutorial/images/stage-2/backend/zeabur-deployment/images/image4.webp)
...
![](/tutorial/images/stage-2/backend/zeabur-deployment/images/image5.webp)
...
![](/tutorial/images/stage-2/backend/zeabur-deployment/images/image6.webp)

点击余额可以查看每日的消耗明细。

![](/tutorial/images/stage-2/backend/zeabur-deployment/images/image7.webp)

现在我们来创建自己的 Dify 服务。首先，在 [控制台首页](https://zeabur.com/projects) 点击 "New Project"。

![](/tutorial/images/stage-2/backend/zeabur-deployment/images/image8.webp)
...
2. **Template（模板）**  
   ...
   ![](/tutorial/images/stage-2/backend/zeabur-deployment/images/image9.webp)
3. **Databases（数据库）**  
   用于部署数据库服务，比如 MySQL、MongoDB 等常见数据库。
   ![](/tutorial/images/stage-2/backend/zeabur-deployment/images/image10.webp)
4. **Functions（函数）**  
   可以部署函数服务，你可以编写 JavaScript 或 Python 代码，让它们以函数的形式被调用。
   ![](/tutorial/images/stage-2/backend/zeabur-deployment/images/image11.webp)

   ![](/tutorial/images/stage-2/backend/zeabur-deployment/images/image12.webp)

5. **Local Project（本地项目）**  
   上传一个本地文件夹，Zeabur 会自动识别其中的启动脚本。这适合将你已经在本地开发好的项目快速部署到 Zeabur 上。
   ![](/tutorial/images/stage-2/backend/zeabur-deployment/images/image13.webp)
6. **Docker Image**  
   部署已经打包好的 Docker 镜像。如果你的项目已经被打成了 Docker 镜像（例如存放在 Docker Hub 或其他镜像仓库中），可以在这里直接部署。
   ![](/tutorial/images/stage-2/backend/zeabur-deployment/images/image14.webp)
7. **Cursor**  
...
![](/tutorial/images/stage-2/backend/zeabur-deployment/images/image15.webp)

接着，输入任意一个名称，Zeabur 会基于这个名称生成一个临时的自定义域名。之后所有人都可以通过这个网址访问你的服务。

![](/tutorial/images/stage-2/backend/zeabur-deployment/images/image16.webp)
...
![](/tutorial/images/stage-2/backend/zeabur-deployment/images/image17.webp)

稍等片刻后，你就能看到 Dify 的登录界面了。输入邮箱地址和注册密码，就可以开始使用你自己的 Dify 服务了。

![](/tutorial/images/stage-2/backend/zeabur-deployment/images/image18.webp)

如果你有兴趣，还可以顺便启动一个 n8n 服务。n8n 也是海外非常流行的一款 AI 工作流平台。

![](/tutorial/images/stage-2/backend/zeabur-deployment/images/image19.webp)![](/tutorial/images/stage-2/backend/zeabur-deployment/images/image20.webp)
...
![](/tutorial/images/stage-2/backend/zeabur-deployment/images/image23.webp)

对于 Trae 来说，生成一个基于 HTML 的贪吃蛇网页游戏非常简单。游戏生成完成后，你只需要按照前面介绍的 Zeabur 本地部署方式，把包含所有文件的文件夹上传上去即可。

![](/tutorial/images/stage-2/backend/zeabur-deployment/images/image24.webp)![](/tutorial/images/stage-2/backend/zeabur-deployment/images/image25.webp)![](/tutorial/images/stage-2/backend/zeabur-deployment/images/image26.webp)

完成后，你就会进入该服务的详情界面：

![](/tutorial/images/stage-2/backend/zeabur-deployment/images/image27.webp)

点击左侧的 "Network" 选项，在页面中找到 "Public Address" 区域。点击 "Generate Domain"，即可生成一个对外访问地址，你可以输入任意喜欢的名称。

![](/tutorial/images/stage-2/backend/zeabur-deployment/images/image28.webp)

![](/tutorial/images/stage-2/backend/zeabur-deployment/images/image29.webp)

生成完成后，只要在浏览器中打开这个地址，就可以运行你自己的贪吃蛇游戏了。其它 HTML 类型的 Web 应用也可以用完全相同的方式来部署。

![](/tutorial/images/stage-2/backend/zeabur-deployment/images/image30.webp)
...
![](/tutorial/images/stage-2/backend/zeabur-deployment/images/image31.webp)

#### 重构为 React 架构

在 Trae 中，你只需要向 Agent 说明："帮我把这份代码重构成 React 架构"，就可以比较轻松地把原本基于 HTML 的结构重构成 React 项目。

![](/tutorial/images/stage-2/backend/zeabur-deployment/images/image32.webp)
...
![](/tutorial/images/stage-2/backend/zeabur-deployment/images/image33.webp)

![](/tutorial/images/stage-2/backend/zeabur-deployment/images/image34.webp)

在网络设置中指定一个访问 URL，方式和部署 HTML 项目时基本相同，就可以启动 React 版本的服务。

![](/tutorial/images/stage-2/backend/zeabur-deployment/images/image35.webp)

![](/tutorial/images/stage-2/backend/zeabur-deployment/images/image36.webp)
...
![](/tutorial/images/stage-2/backend/zeabur-deployment/images/image21.webp)

进入设置页面后，将页面拉到最下方，你会看到类似下面的界面：

![](/tutorial/images/stage-2/backend/zeabur-deployment/images/image22.webp)
